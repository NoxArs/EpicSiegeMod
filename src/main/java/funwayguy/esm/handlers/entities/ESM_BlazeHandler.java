...
public class ESM_BlazeHandler {
    ...
    public static void onEntityJoinWorld(EntityBlaze blaze) {
        ...
        // 不要每次都清零：只在不存在时初始化
        if (!blaze.getEntityData()
            .hasKey(NBT_FIREBALLS)) {
            blaze.getEntityData()
                .setInteger(NBT_FIREBALLS, 0);
        }
    }
    ...
    public static void onLivingUpdate(EntityBlaze blaze) {
        ...
        if (fireballs > 1) {
            int extra = blaze.getEntityData()
                .getInteger(NBT_FIREBALLS);
            ...
            if (fireballs < 5 && extra < ESM_Settings.BlazeFireballs) {
                try {
                    ...
                    blaze.getEntityData()
                        .setInteger(NBT_FIREBALLS, extra + 1);
                } catch (Throwable t) {
                    // ignore
                }
            } else {
