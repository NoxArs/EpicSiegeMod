...
public class ESM_EndermanHandler
{
            ...
            if (getBool(enderman, NBT_LOOKED_AWAY))
            {
                ...
                if (distSq <= 64.0D && getInt(enderman, NBT_TELEPORT_CD) <= 0)
                {
                    ...
                    if (enderman.getRNG().nextInt(20) == 0)
                    {
                        ...
                        if (ESM_Settings.EndermanPlayerTele && enderman.getRNG().nextInt(4) == 0)
                        {
    ...
    public static boolean shouldAttackTarget(EntityEnderman enderman, EntityLivingBase target)
    {
        ...
        Vec3 look = target.getLook(1.0F).normalize();
    ...
    {
        double x = target.posX + (target.getRNG().nextDouble() - 0.5D) * TELEPORT_RANGE_H;
        double z = target.posZ + (target.getRNG().nextDouble() - 0.5D) * TELEPORT_RANGE_H;
        double yBase = target.posY + (target.getRNG().nextInt(TELEPORT_RANGE_V * 2) - TELEPORT_RANGE_V);
    ...
    public static boolean teleportTargetTo(EntityLivingBase target, double x, double yBase, double z)
    {
        ...
        AxisAlignedBB box = AxisAlignedBB.getBoundingBox(
            x - halfW, finalY, z - halfW,
            x + halfW, finalY + target.height, z + halfW
        );

        if (!w.getCollidingBoundingBoxes(target, box).isEmpty()) return false;
    ...
    {
        return e.getEntityData().getBoolean(key);
    }

    private static void setBool(Entity e, String key, boolean v)
    {
        e.getEntityData().setBoolean(key, v);
    }

    private static int getInt(Entity e, String key)
    {
        return e.getEntityData().getInteger(key);
    }

    private static void setInt(Entity e, String key, int v)
    {
        e.getEntityData().setInteger(key, v);
    }

    private static void decNBT(Entity e, String key)
    {
        int v = e.getEntityData().getInteger(key);
        if (v > 0) e.getEntityData().setInteger(key, v - 1);
    }
}
