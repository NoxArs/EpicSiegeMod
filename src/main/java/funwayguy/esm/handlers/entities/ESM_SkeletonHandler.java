...
public class ESM_SkeletonHandler {
    ...
    public static void onLivingUpdate(EntitySkeleton skeleton) {
        ...
        if (skeleton.getEntityData()
            .getBoolean("ESM_SKELETON_SETUP")) return;
        skeleton.getEntityData()
            .setBoolean("ESM_SKELETON_SETUP", true);
        ...
        if (rarity > 0 && skeleton.getRNG()
            .nextInt(rarity) != 0) return;
        ...
        skeleton.getEntityData()
            .setBoolean("ESM_IS_PSEUDO_WITHER_SKELETON", true);
    }
}
